...
data.mat = foreach(i=1:nrow(data.raw), .combine = "rbind") %do% {
  ...
  # read basic name of the character
  charName = as.character(data.array[1])
  ...
  matrix(c(charTitle, charName, charAttributes, charKeywords, charPosition, charLimited), nrow=1, ncol=6)
}

# record column names
colnames(data.mat) = c("charTitle", "charName", "charAttributes", "charKeywords", "charPosition", "charLimited")
