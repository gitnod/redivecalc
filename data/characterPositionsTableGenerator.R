...
data.mat = foreach(i=1:nrow(data.raw), .combine = "rbind") %do% {
  ...
  # record characters location
  charLocation = (charPosition > 300) + (charPosition > 600)
  # return the rearranged row
  matrix(c(charTitle, charAttributes, charKeywords, charPosition, charLocation), nrow=1, ncol=5)
}

# record column names
colnames(data.mat) = c("charTitle", "charAttributes", "charKeywords", "charPosition", "charLocation")
